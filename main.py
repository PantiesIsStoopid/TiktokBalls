...
class Ball:
    ...
    def draw(self):
        pygame.draw.circle(screen, self.color, (int(self.x_pos), int(self.y_pos)), BALL_RADIUS)
    ...
    def check_circle_collision(self):
        ...
        distance = math.sqrt((self.x_pos - CIRCLE_CENTER[0]) ** 2 + (self.y_pos - CIRCLE_CENTER[1]) ** 2)

        # Check if the ball is outside the circle
        if distance > (CIRCLE_RADIUS - BALL_RADIUS):
            # Calculate the reflection vector
            angle = math.atan2(self.y_pos - CIRCLE_CENTER[1], self.x_pos - CIRCLE_CENTER[0])
            self.x_speed = -self.x_speed * BOUNCINESS  # Reverse x-speed and apply bounciness
            self.y_speed = -self.y_speed * BOUNCINESS  # Reverse y-speed and apply bounciness
...
pygame.display.set_caption("Bouncing Balls with No Energy Loss")
...
video_writer = cv2.VideoWriter('bouncing_balls_no_energy_loss.avi', fourcc, VIDEO_FPS, (WIDTH, HEIGHT))
